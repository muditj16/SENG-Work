...
public class FileService {
     ...
     *
     * @param id -> file id.
     * @throws BusinessException 400 if {@code id} is not a valid id of a file in the system;
     * or 500 if the file couldn't be retrieved from the system.
     * @return the saved file.
     */
    @PreAuthorize("hasAnyRole('ADMIN', 'EMPLOYEE')")
    public DownloadFileDto downloadFile(final Long id) {
        final var rootLocation = Paths.get(this.FILES_BASIC_FOLDER_PATH);
        final var fileEntity = this.fileRepository.findById(id)
                .orElseThrow(() -> new BusinessException(HttpStatus.NOT_FOUND, String.format("No existing file with id %s", id)));
        try {
            final Path file = rootLocation.resolve(fileEntity.getPath());
            final Resource resource = new UrlResource(file.toUri());
            if (resource.exists() || resource.isReadable()) {
                return constructDownloadFileEntity(fileEntity, resource);
            } else {
                throw new BusinessException(HttpStatus.INTERNAL_SERVER_ERROR,
                        String.format("Could not read file: %s", fileEntity.getName()));
            }
    ...
    private DownloadFileDto constructDownloadFileEntity(final File fileEntity, final Resource resource) {
