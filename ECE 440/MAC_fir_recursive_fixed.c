...
// ARM MAC-based fixed multiplication with TRUNCATION (no rounding)
int16_t fixed_multiplication_mac_truncate(int32_t a, int16_t b) {
    int64_t temp = arm_mac_accumulate(a, (int32_t)b, 0); // NO rounding bias
    return (int16_t)(temp >> 15); // Just truncate
}

// Standard fixed multiplication with TRUNCATION
int16_t fixed_multiplication_truncate(int32_t a, int16_t b) {
    int32_t temp = (int64_t)a * (int32_t)b;
    // NO rounding bias added
    return (int32_t)(temp >> 15); // Just truncate
}
